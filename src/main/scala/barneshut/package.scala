...
package object barneshut {
  ...
  extends Quad {
    val (mass, massX, massY) = (bodies.map(_.mass).sum : Float, bodies.map(b => b.mass * b.x).sum / mass : Float, bodies.map(b => b.mass * b.y).sum / mass : Float)
    val total: Int = bodies.size
    def insert(b: Body): Quad = if (size <= 1) Leaf(centerX, centerY, size, bodies :+ b) else {
      Fork(this, Empty(centerX + size, centerY, size), Empty(centerX, centerY + size, size), Empty(centerX + size, centerY + size, size)).insert(b)
    }
  }
